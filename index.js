...
const displayBanner = () => {
  console.log(
    chalk.green(
      figlet.textSync("AirdropInsiders", { horizontalLayout: "default" })
    )
  );
};
...
const createStatsTable = (accounts) => {
  ...
  accounts.forEach((account) => {
    table.push([
      ...
      account.email || "-",
      account.proxyConfig
        ? `${account.proxyConfig.type}://${account.proxyConfig.host}:${account.proxyConfig.port}`.substring(
            0,
            20
          ) + "..."
        : "Direct",
  ...
  try {
    ...
    if (useProxy) {
      try {
        ...
        console.log(
          chalk.yellow(
            "No proxy.txt found or error reading proxies. Running without proxies."
          )
        );
      }
    }
...
const main = async () => {
  ...
  spinner.succeed(
    `Loaded ${tokens.length} tokens and ${proxies.length} proxies`
  );
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      try {
        ...
        logSuccess(
          i + 1,
          `Ping successful (${
            account.proxyConfig ? account.proxyConfig.type : "Direct"
          })`,
          stats.pointsToday,
          stats.totalPoints,
          account.username,
        ...
        console.log(
          chalk.red(
            `[${getTimestamp()}] Account ${i + 1}: Error - ${error.message}`
          )
        );
      }
